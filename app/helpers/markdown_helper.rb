module MarkdownHelper
  def markdown(should_render_html: false, current_post_id: nil, posted_by_user: nil, &block)
    @posted_by_user = posted_by_user
    @post = Post.find_by(id: current_post_id)
    @text = yield.to_s.dup
    ...
    parse_directive_poll if @post_id.present?
    clean_up_html
  ...
  def render_poll(post_id)
    return unless @post.present? && @post.id.to_i == post_id.to_i
    poll = @post.poll
    return if poll.nil?
  ...
  def escape_html_characters(should_render_html)
    @text.gsub!("&", "&amp;") # Escape ALL & - prevent Unicode injection / unexpected character behavior
    @text.gsub!("<script", "&lt;script") # Escape <script> Tags
  ...
  def invite_tagged_users
    return unless @posted_by_user.present?
    @text.gsub!(/@([^ \`\@]+)/) do |username_tag|
      tagged_user = User.by_username($1)
      if tagged_user.present? && (tagged_user.friends?(@posted_by_user) || tagged_user == @posted_by_user)
